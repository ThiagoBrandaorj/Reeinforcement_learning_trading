...
def plot_aggregated_results(all_metrics, all_returns_history):
    ...
    plt.title("Distribuição dos retornos finais (últimos 5 episódios)")
...
def main(return_results=False):
    print(" " if return_results else "Execução com gráficos")
    start = time.time()
...
def run_multiple_executions(n_runs=10):
    print(f"\nRodando {n_runs} execuções\n")

    metrics = {k: [] for k in ["MC First Visit", "SARSA", "Q-Learning"]}
    returns_history = {k: [] for k in ["MC First Visit", "SARSA", "Q-Learning"]}
